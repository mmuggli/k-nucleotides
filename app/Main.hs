...
main = do
    ...
    -- utilities for task one and two
    ...
    -- each thread strides across the sequence, building a dict for its particular subset of k-mers, then returns that dict as a list of pairs
    -- these will be small in number and can then be merged into a new dict (which uses a strict Data.Map)
